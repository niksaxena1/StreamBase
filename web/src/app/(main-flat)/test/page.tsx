...
export default async function TestExperimentsPage() {
  ...
  const [{ data: histRaw, error: histErr }, { data: runsRaw, error: runsErr }, { data: plRaw, error: plErr }] =
    await Promise.all([
      ...
      svc
        ...
        .limit(45),
      svc.from("playlists").select("playlist_key,display_name").order("display_order", { ascending: true, nullsFirst: false }),
    ]);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 pb-24">
      ...
      <TestExperimentsClient history={history} runs={runs} playlists={playlists} />
    </div>
  );
}
