-- Create RPC to fetch negative daily streams for a specific run date (health warning)
CREATE OR REPLACE FUNCTION public.health_negative_daily_streams(run_date text)
RETURNS TABLE(
  isrc text,
  name text,
  artist_names text[],
...
AS $$
  ...
  WHERE today.date = run_date::date
    AND today.streams_cumulative < prev.streams_cumulative
  ORDER BY daily_streams_delta ASC;
$$;

GRANT EXECUTE ON FUNCTION public.health_negative_daily_streams(text) TO anon, authenticated;
