-- Multi-playlist graph scope: union (any), intersection (all), or exclude (none) of current memberships.
-- When p_scope_playlists is non-empty, it takes precedence over p_playlist_key.
...
GRANT EXECUTE ON FUNCTION public.artist_collaboration_graph(text, boolean, text[], text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.artist_collaboration_graph(text, boolean, text[], text) TO service_role;
