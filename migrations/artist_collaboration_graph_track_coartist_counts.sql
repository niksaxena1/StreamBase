-- Add per-node co-artist counts from actual track credits (not graph edges).
-- Fixes "0 collaborators" when hide_non_primary is on: edges only link two "primary-eligible"
-- artists, so features-only collaborators were invisible to degree-based filters.

DROP FUNCTION IF EXISTS public.artist_collaboration_graph();
DROP FUNCTION IF EXISTS public.artist_collaboration_graph(text);
DROP FUNCTION IF EXISTS public.artist_collaboration_graph(text, boolean);
...
COMMENT ON FUNCTION public.artist_collaboration_graph(text, boolean) IS
  'Collaboration graph JSON. co_artists_any_track = distinct other credited artists on same scoped ISRC; co_artists_primary_tracks = others on ISRCs where this artist is primary. Use these for "solo" filters when hide_non_primary hides edge-only features.';
