-- Artificial stream spike detection: health_config thresholds, home RPC, user preference column.
--
-- Same-day-of-week baseline: compare each day's daily streams to the average of the prior
-- same weekdays (up to 4 rows before current), using track_daily_streams_effective_public.

INSERT INTO public.health_config (key, value_numeric, description) VALUES
  (
    'artificial_streams_spike_ratio',
    1.25,
    'Flag when daily streams exceed this multiple of the same-weekday baseline average (default 1.25).'
  ),
  (
    'artificial_streams_min_baseline',
    50,
    'Minimum same-DOW average daily streams required before spike detection (default 50).'
  ),
  (
    'artificial_streams_lookback_weeks',
    4,
    'Number of prior same-weekday samples used for baseline (window size; default 4).'
  ),
  (
    'artificial_streams_new_track_grace_days',
    14,
    'Skip tracks whose first_seen is within this many days of the snapshot date (default 14).'
  ),
  (
    'artificial_streams_threshold_crossing_max',
    1500,
    'Treat as Spotify sub-1k display threshold when prev cumulative is 0 and today cumulative is at or below this (default 1500).'
  ),
  (
...
-- Per-user default spike ratio for home dashboard (nullable = fall back to health_config).
ALTER TABLE IF EXISTS public.user_settings
  ADD COLUMN IF NOT EXISTS artificial_streams_spike_ratio numeric;

COMMENT ON COLUMN public.user_settings.artificial_streams_spike_ratio IS
  'Preferred artificial-stream spike ratio threshold for home section; NULL uses health_config artificial_streams_spike_ratio.';
