-- Optional date window for spike detection: when both p_start_date and p_end_date are set,
-- only rows in that range participate in daily series and same-weekday baselines (excludes noisy early data).

DROP FUNCTION IF EXISTS public.home_artificial_stream_spikes(numeric, numeric, integer, bigint, boolean);
...
GRANT EXECUTE ON FUNCTION public.home_artificial_stream_spikes(
  numeric, numeric, integer, bigint, boolean, date, date
