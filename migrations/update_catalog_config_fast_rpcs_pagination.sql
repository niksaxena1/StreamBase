-- Add explicit pagination to /catalog/config fast RPCs.

DROP FUNCTION IF EXISTS public.catalog_config_track_rows(integer);
DROP FUNCTION IF EXISTS public.catalog_config_artist_rows(integer);

CREATE OR REPLACE FUNCTION public.catalog_config_track_rows(
...
AS $$
  ...
  active_distro_memberships AS (
    ...
    JOIN selected_tracks st ON st.isrc = m.isrc
...
CREATE OR REPLACE FUNCTION public.catalog_config_artist_rows(
  track_limit_rows integer DEFAULT 5000,
  result_limit_rows integer DEFAULT 1000,
  result_offset_rows integer DEFAULT 0
)
RETURNS TABLE (
  id text,
  name text,
...
AS $$
  ...
  selected_tracks AS (
    ...
    FROM public.tracks t
    ORDER BY t.last_seen DESC NULLS LAST, t.isrc ASC
    LIMIT GREATEST(COALESCE(track_limit_rows, 5000), 0)
  ),
  track_stats AS (
    SELECT
      t.isrc,
  ...
  FROM artist_rows
  ORDER BY name ASC
  LIMIT GREATEST(COALESCE(result_limit_rows, 1000), 0)
  OFFSET GREATEST(COALESCE(result_offset_rows, 0), 0);
$$;

GRANT EXECUTE ON FUNCTION public.catalog_config_track_rows(integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.catalog_config_artist_rows(integer, integer, integer) TO authenticated;
