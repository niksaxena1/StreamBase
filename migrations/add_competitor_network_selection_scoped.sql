-- Network multi-select helpers for competitor.tracks / competitor.playlist_memberships.

CREATE OR REPLACE FUNCTION competitor.network_selection_scoped_track_totals(
  p_artist_ids text[],
...
AS $$
  ...
  latest AS (
    SELECT max(date)::date AS d FROM competitor.track_daily_streams
  ),
  previous AS (
...
CREATE OR REPLACE FUNCTION competitor.network_selection_scoped_isrcs(
  p_artist_ids text[],
  p_playlist_key text DEFAULT NULL,
  p_hide_non_primary boolean DEFAULT false,
...
GRANT EXECUTE ON FUNCTION competitor.network_selection_scoped_track_totals(text[], text, boolean) TO service_role;
GRANT EXECUTE ON FUNCTION competitor.network_selection_scoped_isrcs(text[], text, boolean, integer, integer) TO service_role;
